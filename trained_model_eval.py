...
# 加载模型
model = load_model(
    r"./GroundingDINO-main/groundingdino/config/GroundingDINO_SwinT_OGC.py",
r"./Open-GroundingDino-main/logs/checkpoint_best_regular.pth",
)
...
text_threshold = 0.25
# 先推理一张图片
image_demo_source, image_demo = load_image(r"./基础推理测试/demo pic.png")
...
annotated_demo_frame = annotate(
    image_source=image_demo_source,
    boxes=boxes,
    logits=logits,
    phrases=phrases
)
cv2.imwrite("./训练结果/eval_on_demo_pic.jpg", annotated_demo_frame)
prompt = "remote. keyboard. cell phone. microwave. oven. toaster. sink. refrigerator. book. clock. vase. scissors. teddy bear. hair drier. toothbrush."
...
#TODO:2.写入json文件
with open(r"./训练结果/training model eval on 15unseens.json",'w') as f:
...
# 加载预测结果文件
with open(r"训练结果/training model eval on 15unseens.json", 'r') as f:
    results = json.load(f)
